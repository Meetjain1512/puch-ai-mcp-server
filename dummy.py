...
TOKEN = os.getenv("AUTH_TOKEN", "5079b331ef08")  # Use environment variable or default
MY_NUMBER = os.getenv("PHONE_NUMBER", "9516326057")  # Use environment variable or defaultport Annotated, Optional
from fastmcp import FastMCP
from fastmcp.server.auth.providers.bearer import BearerAuthProvider, RSAKeyPair
import markdownify
from mcp import ErrorData, McpError
...
TOKEN = "5079b331ef08"  # Replace with your actual application key from Puch
MY_NUMBER = "919516326057"  # Replace with your number in format {country_code}{number}
...
async def resume() -> str:
    ...
    This function:
    1. Finds and reads your resume from the local file.
    2. Returns the resume as markdown text.
    ...
    try:
        ...
        if not resume_path.exists():
            return "# Resume Not Found\n\nPlease create a resume.md file in the same directory as this server."
...
async def main():
    port = int(os.getenv("PORT", 8085))  # Railway sets PORT automatically
    await mcp.run_async(
        "streamable-http",
        host="0.0.0.0",
        port=port,
